...
- **Auth is session-based; `Item.sellerId` stays optional.** `POST /signup` (bcryptjs hash, `@keio.jp` only) and `POST /login`/`POST /logout` manage `req.session.userId`; a global middleware loads `req.session.userId` into `req.user` on every request. Listing an item requires `req.user` (enforced server-side in `POST /items`, not just hidden in the UI). `Item.sellerId` is still an *optional* FK to `User` — pre-auth listings (created before this feature existed) have `sellerId: null` and are left as-is by design, so don't assume every `Item` has a `seller`. `sellerName`/`sellerContact` remain free-text fields on `Item` (pre-filled from the logged-in user's name/email but editable) rather than being derived from the `User` relation.
- **Sessions are stored in Postgres**, not memory: `connect-pg-simple` (`createTableIfMissing: true`) creates/uses a `session` table in the same DB as `DATABASE_URL`, so login state survives Render restarts/redeploys. `SESSION_SECRET` is read from env — must be set in both `.env` and the Render dashboard.
...
- **Env/config**: `DATABASE_URL` and `SESSION_SECRET` live in `.env` (gitignored, never commit it); `DATABASE_URL` must include `?sslmode=require` for the Render Postgres instance. `prisma.config.ts` (not `schema.prisma`) is what wires `DATABASE_URL` into the Prisma CLI via `dotenv/config`.
- **Deployment**: Render Web Service at `webpro-ygpb.onrender.com`. Render's Build Command is `npm ci` and Start Command is `npm start`; `DATABASE_URL`/`SESSION_SECRET` are set directly in the Render dashboard, not from a committed file.
